...
function aiPromptPayload(payload: { extractedText: string; knownBuildings?: string[]; knownFlats?: Array<{ id: string; flatLabel?: string; street?: string; streetId?: string; buildingNo?: string; apartmentNo?: string }>; }) {
  return JSON.stringify({ knownBuildings: payload.knownBuildings || [], knownFlats: payload.knownFlats || [], extractedText: String(payload.extractedText || "").slice(0, 120000) });
}
...
function parseBasicInvoiceText(text: string, knownBuildings?: string[], knownFlats?: FlatCandidate[]): Partial<InvoiceOCRResult> {
  ...
  if (addressMatch?.flatId) allocationType = "FLAT";
  else if (allocationType === "UNKNOWN" && suggestedBuildingId) allocationType = "BUILDING";
...
export async function analyzeInvoiceText(payload: { extractedText: string; knownBuildings?: string[]; knownFlats?: Array<{ id: string; flatLabel?: string; street?: string; streetId?: string; buildingNo?: string; apartmentNo?: string }>; }) {
...
export async function analyzeInvoicePdf(payload: { filename: string; mimeType: string; buffer: Buffer; extractedText: string; knownBuildings?: string[]; knownFlats?: Array<{ id: string; flatLabel?: string; street?: string; streetId?: string; buildingNo?: string; apartmentNo?: string }>; }) {
...
export async function analyzeInvoiceImage(payload: { mimeType: string; buffer: Buffer; knownBuildings?: string[]; knownFlats?: Array<{ id: string; flatLabel?: string; street?: string; streetId?: string; buildingNo?: string; apartmentNo?: string }>; }) {
