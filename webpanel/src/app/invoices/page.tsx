...
export default function InvoicesPage() {
  ...
  const stats = useMemo(() => ({
    total: items.filter((x) => !x.archivedAtMs && !x.deletedAtMs).length,
  ...
  async function bulkAutoStage() {
    if (!communityId) return;
    const candidates = items.filter((x) => !x.archivedAtMs && !x.deletedAtMs && String(x.status || "") !== "DELETED");
    ...
    try {
      ...
      for (const inv of candidates) {
        try {
          const cardStreetMap = new Map(streets.map((s) => [s.id, s.name]));
          ...
          if (!buildingId && candidateFlats.length === 1) buildingId = String(candidateFlats[0].buildingNo || "");
          if (!buildingId && candidateFlats.length > 1) buildingId = String(candidateFlats[0].buildingNo || "");
          ...
          await callable("approveInvoice")({
            ...
            assignment: {
              ...
              category: inv.parsed?.category || inv.category || "INNE",
              scope,
      ...
      setMsg(`Automatyczne rozliczenie zakończone. Przetworzono: ${processed}, błędy: ${errors}.`);
    } finally {
      setBulkBusy(false);
    }
  }
    ...
    <RequireAuth roles={["MASTER", "ACCOUNTANT"]}>
      ...
      <div style={{ padding: 24, display: "grid", gap: 16, minWidth: 0 }}>
        ...
        <div style={{ display: "grid", gap: 12, minWidth: 0 }}>
          {items.filter((x) => !x.archivedAtMs).map((inv) => <InvoiceCard key={`${docCollectionName(inv)}-${inv.id}`} inv={inv} communityId={communityId} flats={flats} streets={streets} />)}
          {items.some((x) => !!x.archivedAtMs) ? <div className="card" style={{ marginTop: 8, display: "grid", gap: 10 }}><h3>Archiwum faktur</h3>{items.filter((x) => !!x.archivedAtMs).map((inv) => <InvoiceCard key={`${docCollectionName(inv)}-${inv.id}`} inv={inv} communityId={communityId} flats={flats} streets={streets} />)}</div> : null}
        </div>
      </div>
    </RequireAuth>
  );
...
function InvoiceCard({ inv, communityId, flats, streets }: { inv: Invoice; communityId: string; flats: Flat[]; streets: StreetOption[] }) {
  ...
  async function archiveOrDelete(patch: Record<string, any>) {
    await updateDoc(doc(db, "communities", communityId, docCollectionName(inv), inv.id), patch);
  }
