...
export default function InvoicesPage() {
  ...
  useEffect(() => {
    ...
    const unsubInv = onSnapshot(query(collection(db, "communities", communityId, "invoices"), orderBy("createdAtMs", "desc")), (snap) => setItems(snap.docs.map((d) => ({ id: d.id, ...(d.data() as any) }))));
    ...
    const unsubFlats = onSnapshot(query(collection(db, "communities", communityId, "flats")), (snap) => {
      ...
      setFlats(snap.docs.map((d) => {
        ...
        return { id: d.id, ...data, street: displayStreetName(data.street || data.streetName, data.streetId, streetMap), apartmentNo: String(data.apartmentNo || data.flatNumber || "") };
      }));
    });
    return () => { unsubInv(); unsubStreets(); unsubFlats(); };
  }, [communityId]);
  ...
  async function handlePdf(file: File) {
    ...
    } catch (error: any) { setMsg(error?.message || "Nie udało się odczytać PDF."); }
  ...
  async function saveOcrDraft() {
    ...
    await addDoc(collection(db, "communities", communityId, "invoices"), {
      vendorName: ocrResult.supplierName || form.vendorName, title: ocrResult.invoiceNumber || form.title || "Faktura z PDF", period,
      ...
      currency: ocrResult.currency || "PLN", source: "PDF_AI", status: ocrResult.needsReview ? "SUGGESTED" : "READY_TO_STAGE",
    ...
    <RequireAuth roles={["MASTER", "ACCOUNTANT"]}>
      ...
      <div style={{ padding: 24, display: "grid", gap: 16, minWidth: 0 }}>
        ...
        <div className="card" style={{ display: "grid", gap: 12, minWidth: 0 }}>
          ...
          <div style={actionRowStyle}><button className="btn" onClick={async () => { await addDoc(collection(db, "communities", communityId, "invoices"), { vendorName: form.vendorName, title: form.title, period: form.period, category: form.category, totalGrossCents: Math.round(Number((form.totalGross || "0").replace(",", ".")) * 100), currency: "PLN", status: "NOWA", source: "MANUAL", createdAtMs: Date.now(), updatedAtMs: Date.now() }); setForm({ ...form, vendorName: "", title: "", totalGross: "" }); }}>Dodaj fakturę</button><button className="btnGhost" onClick={async () => { await callable("ksefFetchInvoices")({ communityId, period: form.period }); setMsg("Pobrano przykładowe faktury KSeF do kolekcji ksefInvoices."); }}>Pobierz mock KSeF</button></div>
        </div>
        {msg ? <div style={{ color: "#8ef0c8", fontWeight: 700 }}>{msg}</div> : null}
        <div style={{ display: "grid", gap: 12, minWidth: 0 }}>
          {items.filter((x) => !x.archivedAtMs).map((inv) => <InvoiceCard key={inv.id} inv={inv} communityId={communityId} flats={flats} streets={streets} />)}
          {items.some((x) => !!x.archivedAtMs) ? <div className="card" style={{ marginTop: 8, display: "grid", gap: 10 }}><h3>Archiwum faktur</h3>{items.filter((x) => !!x.archivedAtMs).map((inv) => <InvoiceCard key={inv.id} inv={inv} communityId={communityId} flats={flats} streets={streets} />)}</div> : null}
        </div>
      </div>
    </RequireAuth>
  );
...
function InvoiceCard({ inv, communityId, flats, streets }: { inv: Invoice; communityId: string; flats: Flat[]; streets: StreetOption[] }) {
  ...
  const streetOptions = useMemo(() => {
    ...
    flats.forEach((f) => {
      const id = String(f.streetId || "").trim();
  ...
  const filteredFlatsByStreet = useMemo(() => flats.filter((f) => String(f.streetId || normalizeStreetId(String(f.street || f.streetName || ""))) === streetId), [flats, streetId]);
  ...
  return (
    <div className="card" style={{ display: "grid", gap: 10, minWidth: 0 }}>
      <div style={cardHeaderStyle}><strong>{inv.vendorName || "Faktura"}</strong><span>{inv.title || inv.id}</span><span>status: {inv.status}</span><span>{fromCents(amount)} PLN</span>{ai ? <span>AI: {Number(ai.confidence || 0).toFixed(2)}</span> : null}</div>
      <div style={formGridStyle}>
        ...
        <select className="select" style={fieldStyle} value={scope} onChange={(e) => setScope(e.target.value)}><option value="COMMON">Części wspólne</option><option value="FLAT">Konkretny lokal</option></select>
        ...
        {showFlatId ? <select className="select" style={fieldStyle} value={flatId} onChange={(e) => setFlatId(e.target.value)}><option value="">Wybierz lokal</option>{flatOptions.map((f) => <option key={f.id} value={f.id}>{flatLabel(f)}</option>)}</select> : null}
      </div>
      <div style={actionRowStyle}>
        ...
        <button className="btnGhost" onClick={async () => { await updateDoc(doc(db, "communities", communityId, "invoices", inv.id), { archivedAtMs: inv.archivedAtMs ? null : Date.now(), updatedAtMs: Date.now() }); }}>{inv.archivedAtMs ? "Przywróć" : "Archiwizuj"}</button>
        <button className="btnGhost" onClick={async () => { if (window.confirm(`Usunąć fakturę ${inv.title || inv.id}?`)) await updateDoc(doc(db, "communities", communityId, "invoices", inv.id), { deletedAtMs: Date.now(), archivedAtMs: Date.now(), status: "DELETED", updatedAtMs: Date.now() }); }}>Kosz</button>
      </div>
