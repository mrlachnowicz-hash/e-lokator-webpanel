...
function sameAccount(a: any, b: any) {
...
export default function PaymentsPage() {
  ...
  useEffect(() => {
    ...
    const u4 = onSnapshot(doc(db, "communities", communityId), (snap) => {
      ...
      const defaults = readCommunityDefaults(data);
      setSettings({
            ...
            return (
              <div key={p.id} className="card" style={{ display: "grid", gap: 8 }}>
                <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
                  <strong>{flatDisplay(flat) || settlement?.flatLabel || "brak dopasowania"}</strong>
                  <span>{p.payerName || settlement?.residentName || "—"}</span>
                  <span>{p.period || "—"}</span>
