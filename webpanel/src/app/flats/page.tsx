  ...
  useEffect(() => {
    ...
    const unsubFlats = onSnapshot(query(collection(db, "communities", communityId, "flats")), async (snap) => {
      ...
      try {
        ...
        const keys = ["seatsTotal", "panelSeats", "panelSeatsLimit", "seats", "seatsLimit", "totalSeats", "maxSeats", "purchasedSeats", "seatsPurchased", "flatsLimit", "localsLimit", "localiLimit", "unitsLimit", "licenses", "seatCount"];
        ...
        const used = typeof usedRaw === "number" ? usedRaw : (typeof usedRaw === "string" && usedRaw.trim() !== "" && Number.isFinite(Number(usedRaw)) ? Number(usedRaw) : snap.size);
