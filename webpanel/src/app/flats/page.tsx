...
function getApartmentNo(flat: Flat) {
  return String(getDisplayApartmentNo(flat));
}
...
export default function FlatsPage() {
  ...
  const [seatInfo, setSeatInfo] = useState<any>({ limit: null, used: 0, remaining: null });
  ...
  useEffect(() => {
    ...
    let payersByFlatId = new Map<string, any>();
    let streetNamesById = new Map<string, string>();

    const merge = () => {
      const merged = flatsCache
        .map((flat) => {
    ...
    const unsubPayers = onSnapshot(query(collection(db, "communities", communityId, "payers")), (snap) => {
      payersByFlatId = new Map(snap.docs.map((d) => [d.id, { id: d.id, ...(d.data() as any) }]));
      merge();
    });
  ...
  const startEdit = (flat: Flat) => {
    setForm({
      ...
      name: String(flat.name || flat.firstName || ""),
      surname: String(flat.surname || flat.lastName || ""),
          ...
          {canEdit ? (
            <div style={{ border: "1px solid #e5e5e5", borderRadius: 12, padding: 16 }}>
              ...
              <div style={{ display: "grid", gap: 10, gridTemplateColumns: "1fr 1fr 1fr" }}>
                ...
                <input placeholder="Nr lokalu" value={form.apartmentNo} onChange={(e) => setForm({ ...form, apartmentNo: e.target.value })} />
                <input placeholder="Imię" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
                <input placeholder="Nazwisko" value={form.surname} onChange={(e) => setForm({ ...form, surname: e.target.value })} />
