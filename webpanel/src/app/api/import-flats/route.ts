...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const row of rows) {
      ...
      const streetId = (await ensureStreetExists(communityId, street, uid)) || streetIdByName.get(street.toLowerCase()) || fallbackStreetId || street;
    ...
    await batch.commit();
    const usedAfter = flatsSnap.size + plannedCreates;
    await communityRef.set({ seatsUsed: usedAfter, updatedAtMs: now }, { merge: true });
    const seatStateEnd = getSeatState({ ...(communityData as any), seatsUsed: usedAfter } as any, usedAfter);
